...
/**
 * This controller serves public resources under /assets. It is capable of serving static files, webjars or files from a completely different directory on the
 * server.
 */
@Singleton
public class FixedAssetsController {

    private final static Logger logger = LoggerFactory.getLogger(FixedAssetsController.class);
    ...
    @Inject
    public FixedAssetsController(AssetsControllerHelper assetsControllerHelper, HttpCacheToolkit httpCacheToolkit, MimeTypes mimeTypes, NinjaProperties ninjaProperties) {
    ...
    /**
     * Serves resources from the assets directory of your application. For instance: route: /robots.txt A request to /robots.txt will be served from
     * /assets/robots.txt. You can also use a path like the following to serve files: route: /assets/{fileName: .*} matches /assets/app/app.css and will return
    ...
    /**
     * Serves resources from the assets directory of your application. For instance: A request to /robots.txt will be served from /assets/robots.txt. Request to
    ...
    /**
     * Loads files from assets directory. This is the default directory of Ninja where to store stuff. Usually in src/main/java/assets/. But if user wants to
     * use a dir outside of application project dir, then base dir can be overridden by static.asset.base.dir in application conf file.
     */
    private URL getStaticFileFromAssetsDir(String fileName) {
        ...
        if (ninjaProperties.isDev()
            ...
            // CHANGED serving from target/classes/assets if not in src/main/java/assets
            boolean urlValid = url != null;
            URLConnection urlConnection = null;
            if (url != null) {
                try {
                    urlConnection = url.openConnection();
            ...
            if (!urlValid) {
                url = getStaticFileFromAssetsDirNonDev(fileName);
            }
        } else {
            url = getStaticFileFromAssetsDirNonDev(fileName);
        }
    ...
    private URL getStaticFileFromAssetsDirNonDev(String fileName) {
        URL url;
